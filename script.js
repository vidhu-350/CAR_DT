...
function updatePhysics(deltaTime) {
    ...
    if (reverse) targetForce = -carState.engineForce * 0.3; // Reduced reverse force

    // Apply strong initial impulse if not moving
    const velocity = carBody.body.getLinearVelocity();
    const currentSpeed = Math.sqrt(velocity.x() * velocity.x() + velocity.z() * velocity.z());
    if (accel && currentSpeed < 0.1) {
        const impulse = new Ammo.btVector3(
            Math.sin(carState.rotation) * 100,
            0,
            Math.cos(carState.rotation) * 100
        );
        carBody.body.applyCentralImpulse(impulse);
        console.log("Applied initial impulse");
    }

    // Apply continuous force
    const forceVector = new Ammo.btVector3(
        Math.sin(carState.rotation) * targetForce,
        0,
        Math.cos(carState.rotation) * targetForce
    );
    carBody.body.applyCentralForce(forceVector);

    // Minimal braking and friction
    let brakingForce = 0;
    if (!accel && !reverse && currentSpeed > 0.1) brakingForce = carState.brakingForce * 0.05; // Very light braking
    carBody.body.setFriction(brakingForce > 0 ? 0.2 : 0.1); // Extremely low friction
    ...
    // Apply steering by adjusting rotation
    if (currentSpeed > 0.1) {
        const steerFactor = carState.currentSteering * (1 / (1 + currentSpeed * 0.1));
        const steerForce = new Ammo.btVector3(0, steerFactor * 500, 0); // Lateral force for turning
        carBody.body.applyTorque(steerForce);
    }
    ...
    if (pos.y() < carState.wheelRadius - 0.05 || pos.y() > carState.wheelRadius + 0.05) {
        ...
        carBody.body.setWorldTransform(transform);
        carBody.body.setLinearVelocity(new Ammo.btVector3(velocity.x(), 0, velocity.z())); // Preserve horizontal velocity
        console.log("Car stabilized to ground level");
    }
