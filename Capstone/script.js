...
async function loadCar() {
  ...
  return new Promise((resolve) => {
    ...
    loader.load(
      'models/car.glb', // Make sure this path is correct
      (gltf) => {
        ...
        if (detected && detected.frontLeft && detected.frontRight && detected.backLeft && detected.backRight) {
          ...
          console.warn('Using fallback wheel positions');
          // Fallback connection points based on bounding box
          ...
          // Create proxy wheels at these positions
          ...
          for (let i = 0; i < 4; i++) {
            ...
            const m = proxies[key];
            m.position.copy(wp);
            m.quaternion.copy(root.quaternion);
            m.userData.isProxy = true;
            wheels[key] = m;
          }
        }

        // Set wheel radius if not defined
        if (!carState.wheelRadius) {
          ...
          if (pick?.geometry) {
            ...
            carState.wheelRadius = Math.max(0.15, Math.min(0.6, diameter * 0.5));
          } else {
            carState.wheelRadius = 0.35;
          }
        }
        ...
        console.log('Final wheel setup:', {
          frontLeft: wheels.frontLeft?.name || 'proxy',
          frontRight: wheels.frontRight?.name || 'proxy',
          backLeft: wheels.backLeft?.name || 'proxy',
          backRight: wheels.backRight?.name || 'proxy',
...
function updatePhysics(deltaTime) {
  ...
  // Engine force
  const engineForce = carState.engineForce;
  let force = 0;
  if (accel)   force = engineForce;
  if (reverse) force = -engineForce * 0.6;
  ...
  for (let i = 0; i < 4; i++) {
    vehicle.setBrake((accel || reverse) ? 0 : brakingForce * 0.1, i);
  }
...
function updateCarVisuals() {
  ...
  for (let i = 0; i < vehicle.getNumWheels(); i++) {
    ...
    // Ensure wheel is in scene (not parented to car)
    ...
    // Create correction quaternion if it doesn't exist
    if (!carState._wheelCorrection) {
      // Default correction for wheels modeled with Z as axle
      carState._wheelCorrection = new THREE.Quaternion().setFromEuler(
        new THREE.Euler(0, 0, Math.PI/2)
      );
    }

    // Apply rotation with correction
    const bulletQuat = new THREE.Quaternion(wq.x(), wq.y(), wq.z(), wq.w());
    mesh.quaternion.copy(bulletQuat.multiply(carState._wheelCorrection));
  }
}
